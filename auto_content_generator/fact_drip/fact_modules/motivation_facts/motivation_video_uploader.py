...
def start_video_uploader(config, log_manager, app_logs, MAX_LOG_ENTRIES, motivation_fact):
    log_message = "Starting Fact Drip Video Uploader..."
    if len(app_logs) >= MAX_LOG_ENTRIES:
        app_logs.pop(0)
    log_manager.append_log(log_message)

    uploaded_file = upload_file('motivation_output_video.mp4',
                                'auto_content_generator/fact_drip/assets/temp/motivation_output_video.mp4',
                                'video/mp4')
    time.sleep(5)
    print(uploaded_file)

    fact_pt1 = motivation_fact[3] + "..."
    youtube_account = ""
    ...
    video_desc = '#dailyfacts #factshorts #motivationfacts'
    link = uploaded_file
            ...
            "posts": [
                {
                    ...
                    "networks": {
                        "default": {
                            ...
                            "title": fact_pt1 + " #motivation #facts",
                            "text": video_desc,
                            "media": [
                                {
                                    "id": media_id,
