...
def start_motivation_chatgpt(config, log_manager, app_logs, MAX_LOG_ENTRIES):
    log_message = "Starting ChatGPT for Motivation Facts..."
    if len(app_logs) >= MAX_LOG_ENTRIES:
        app_logs.pop(0)
    log_manager.append_log(log_message)

    openai.api_key = config['openai']['api_key']
    system_msg = config['chatgpt']['motivation_prompt_categories']
    user_msg = ""
    ...
    for category in categories_dict["motivation_categories"]:
        facts_attempt = 0
        while facts_attempt < max_attempts:
            system_msg_for_facts = f"Create me 50 sets of interesting motivation facts with the category \"{category}\". " + config['chatgpt']['motivation_prompt_facts']
        ...
        for fact_key, fact_values in facts_dict.items():
            ...
            if not check_fact_duplicate(fact_category, fact_pt1, fact_pt2):
                insert_fact("motivation", fact_category, fact_pt1, fact_pt2)
