...
def start_facebook_upload(uploaded_image_thumbnail, facebook_contents, app_logs, MAX_LOG_ENTRIES, log_manager):
            ...
            "posts": [
                {
                    ...
                    "networks": {
                        "default": {
                            "type": "photo",
                            "text": str(facebook_contents) + " #News",
                            "media": [
                                {
                                    "id": payload_id,
                                    "name": "",
    ...
    while True:
        ...
        if response.status_code == 200:
            try:
                ...
                if status == "complete":
                    print("Completed.")
                    log_message = "Article Successfully Uploaded to Mock Times Facebook!"
                ...
                print("Failed to parse JSON response.")
                log_message = "There was a problem uploading article to Facebook..."
                if len(app_logs) >= MAX_LOG_ENTRIES:
                    app_logs.pop(0)
                log_manager.append_log(log_message)
